...
async def build_package(request: BuildRequest):
    ...
    try:
        ...
        if not dev_package:
            raise HTTPException(status_code=400, detail="No valid package found at source path")
        ...
        return {
            ...
            "build_path": str(build_result.build_path) if hasattr(build_result, 'build_path') else None,
            "install_path": str(build_result.install_path) if hasattr(build_result, 'install_path') else None,
...
async def release_package(request: ReleaseRequest):
    ...
    try:
        ...
        return {
            ...
            "package": dev_package.name,
            "version": str(dev_package.version),
            "released_packages": [str(pkg.uri) for pkg in release_result.released_packages] if hasattr(release_result, 'released_packages') else [],
            "message": request.release_message,
        }
    except HTTPException:
        raise
...
@router.get("/systems")
async def get_build_systems():
    """Get available build systems."""
    try:
...
async def get_package_variants(source_path: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get package variants: {e}")
...
async def get_build_dependencies(source_path: str):
    ...
    try:
        ...
        dependencies = {
            ...
            "build_requires": [str(req) for req in getattr(dev_package, 'build_requires', [])],
            "private_build_requires": [str(req) for req in getattr(dev_package, 'private_build_requires', [])],
        }
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get build dependencies: {e}")
