...
async def copy_package(request: PackageCopyRequest):
    ...
    try:
        ...
        if not dest_repo:
            raise HTTPException(status_code=404, detail=f"Destination repository not found: {request.dest_repository}")
...
async def remove_package(request: PackageRemoveRequest):
    ...
    try:
        ...
        if request.version:
            ...
            if not package:
                raise HTTPException(status_code=404, detail=f"Package {request.package_name}-{request.version} not found")
            ...
            if not packages:
                raise HTTPException(status_code=404, detail=f"Package family {request.package_name} not found")
...
async def get_package_from_uri(package_uri: str):
    ...
    try:
        ...
        if not package:
            raise HTTPException(status_code=404, detail=f"Package not found at URI: {package_uri}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get package from URI: {e}")
...
async def get_variant_from_uri(variant_uri: str):
    ...
    try:
        ...
        if not variant:
            raise HTTPException(status_code=404, detail=f"Variant not found at URI: {variant_uri}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get variant from URI: {e}")


@router.get("/help/{package_name}")
async def get_package_help(package_name: str, version: Optional[str] = None):
...
async def get_package_tests(package_name: str, version: Optional[str] = None):
