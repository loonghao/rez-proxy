...
class SuiteAddContextRequest(BaseModel):
    ...
    environment_id: str = Field(..., description="ID of the resolved environment to add")
    prefix_char: Optional[str] = Field(None, description="Prefix character for tools")
...
async def get_suite(suite_id: str) -> SuiteInfo:
    ...
    try:
        ...
        try:
            ...
            tools = {name: {"context": tool.context_name, "command": str(tool)} 
...
@version(1)
async def add_context_to_suite(suite_id: str, request: SuiteAddContextRequest) -> Dict[str, str]:
    ...
    try:
        ...
        suite.add_context(
            ...
            context=context,
            prefix_char=request.prefix_char
        )
        
        return {"message": f"Context '{request.context_name}' added to suite '{suite_id}'"}
...
@version(1)
async def alias_tool_in_suite(suite_id: str, request: SuiteToolAliasRequest) -> Dict[str, str]:
...
@version(1)
async def save_suite(suite_id: str, path: Optional[str] = None) -> Dict[str, str]:
    ...
    try:
        ...
        return {
            "message": f"Suite '{suite_id}' saved to '{path}'",
            "path": path
        }
    except Exception as e:
