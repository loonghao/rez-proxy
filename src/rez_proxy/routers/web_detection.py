...
from rez_proxy.core.web_detector import clear_detection_cache, get_web_detector
...
class WebDetectionInfo(BaseModel):
    ...
    is_web_environment: bool = Field(
        ..., description="Whether web environment is detected"
    )
    service_mode: ServiceMode = Field(..., description="Detected service mode")
    forced_mode: ServiceMode | None = Field(
        None, description="Manually forced service mode"
    )
    detection_methods: dict[str, bool] = Field(
        ..., description="Results of individual detection methods"
    )
...
async def get_web_detection_info() -> WebDetectionResponse:
    ...
    try:
        ...
        web_info = WebDetectionInfo(
            ...
            forced_mode=ServiceMode(detection_info["forced_mode"])
            if detection_info["forced_mode"]
            else None,
...
async def get_web_compatibility_info() -> dict[str, Any]:
    ...
    api_compatibility = {
        ...
        "system": {
            ...
            "limitations": [
                "Local Rez installation detection",
    ...
    compatibility_stats = {
        ...
        "full_compatible": len(
            [
                api
                for api in api_compatibility.values()
                if api["level"] == CompatibilityLevel.FULL.value
            ]
        ),
        "partial_compatible": len(
            [
                api
                for api in api_compatibility.values()
    ...
    return {
        ...
        "compatibility_overview": {
            ...
            "compatibility_percentage": round(
                (
                    compatibility_stats["full_compatible"]
                    / compatibility_stats["total_api_groups"]
                )
                * 100,
                1,
            ),
